...
const loadQuestions = () => { try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]') } catch { return [] } }
...
const storedById = new Map(storedQuestions.filter(q => !q._deleted).map(q => [q.id, q]))
const deletedIds = new Set(storedQuestions.filter(q => q._deleted).map(q => q.id))
const initialQuestions = baseQuestions.filter(q => !deletedIds.has(q.id)).map(q => storedById.get(q.id) ? { ...q, ...storedById.get(q.id), subject: storedById.get(q.id).subject || q.subject } : q)
const customOnly = storedQuestions.filter(q => !q._deleted && !baseQuestions.some(base => base.id === q.id))
...
const syncBank = items => {
  ...
  items.forEach(q => { if (!questionBank[q.subject]) questionBank[q.subject] = []; questionBank[q.subject].push(q) })
}
...
export default function Admin({ onHome }) {
  ...
  const submit = e => {
    ...
    const item = { ...clean, id: editingId || `custom-${Date.now()}` }
    const next = editingId ? questions.map(q => q.id === editingId ? item : q) : [item, ...questions]
    persist(next)
    setMessage(editingId ? 'Question updated successfully.' : 'Question added successfully.')
    resetForm(); setTimeout(() => setMessage(''), 2200)
  }
  ...
  return <div className="admin-page">
    ...
    <main className="admin-main">
      <section className="admin-intro"><div><p className="admin-kicker">BPSC TRE · COMPUTER SCIENCE</p><h2>{editingId ? 'Edit question' : 'Add new question'}</h2><p>Saare 100-question subject banks yahan show honge. Kisi bhi question ko edit, duplicate ya delete kar sakte hain.</p></div><div className="admin-count"><strong>{questions.length}</strong><span>Total questions</span></div></section>
