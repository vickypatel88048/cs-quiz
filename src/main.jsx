...
function App(){
  ...
  const startQuiz=categoryId=>{setSelectedCategory(categoryId||'exam-practice');setView('quiz')}
  return <>
    {view==='home'&&<Home language={language} onLanguageChange={changeLanguage} onStart={()=>startQuiz('exam-practice')} onCategories={()=>setView('categories')} onNotes={()=>setView('notes')}/>} 
    {view==='categories'&&<Categories language={language} onLanguageChange={changeLanguage} onStart={startQuiz} onHome={()=>setView('home')}/>} 
    {view==='notes'&&<Notes onHome={()=>setView('home')}/>} 
