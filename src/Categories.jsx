...
export default function Categories({ onStart, onHome }) {
  ...
  return <div className="site-shell" id="top">
    <header className="navbar">
      <button className="brand brand-button" onClick={onHome}><span className="brand-mark"><BrainCircuit size={21} /></span><span>CS<span className="brand-accent">Quiz</span></span></button>
      <nav className={menuOpen ? 'nav-links is-open' : 'nav-links'}><button className="nav-home-btn" onClick={onHome}>Home</button><a className="active" href="#categories" onClick={() => setMenuOpen(false)}>Categories</a><a href="#exam" onClick={() => setMenuOpen(false)}>Exam Practice</a><a href="#about" onClick={() => setMenuOpen(false)}>About</a></nav>
    ...
    <main>
      ...
      <section className="exam-banner" id="exam"><div><p className="kicker">Competitive preparation</p><h2>Ready for exam-style practice?</h2><p>Build speed and accuracy for UGC NET, GATE, STET and placement preparation.</p></div><button onClick={() => onStart('exam-practice')}>Start quiz <ArrowRight size={16} /></button></section>
