...
import { ArrowLeft, ArrowRight, BrainCircuit, Search, Menu, X, Monitor, GitBranch, Database, Cpu, Network, Code2, Binary, Braces, Blocks, ShieldCheck, GraduationCap, Clock3, CheckCircle2, Settings } from 'lucide-react'
...
export default function Categories({ onStart, onHome }) {
  ...
  const filtered = subjectCategories.filter(c => (difficulty === 'All' || c.difficulty === difficulty) && `${c.name} ${c.description}`.toLowerCase().includes(search.toLowerCase()))
  const totalQuestions = subjectCategories.reduce((sum, c) => sum + c.count, 0)

  if (adminOpen) return <Admin onHome={() => setAdminOpen(false)} />
  ...
  return <div className="site-shell" id="top">
    <header className="navbar">
      ...
      <nav className={menuOpen ? 'nav-links is-open' : 'nav-links'}><button className="nav-home-btn" onClick={onHome}>Home</button><a className="active" href="#categories" onClick={() => setMenuOpen(false)}>Subjects</a><a href="#mock" onClick={() => setMenuOpen(false)}>Full Mock</a><a href="#about" onClick={() => setMenuOpen(false)}>About</a><button className="admin-nav-btn" onClick={() => { setAdminOpen(true); setMenuOpen(false) }}><Settings size={14}/> Admin</button></nav>
      <div className="nav-actions"><button className="admin-top-btn" onClick={() => setAdminOpen(true)}><Settings size={15}/> Admin</button><button className="mobile-menu" onClick={() => setMenuOpen(v => !v)} aria-label="Open menu">{menuOpen ? <X size={20} /> : <Menu size={20} />}</button></div>
    </header>
    <main>
      ...
      <section className="category-section" id="categories"><div className="section-heading"><div><p className="kicker">BPSC TRE · Computer Science</p><h2>Practice by subject</h2></div><p className="result-count">{filtered.length} subjects</p></div><div className="category-tools"><label className="category-search"><Search size={17} /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search CS subjects..." /></label><div className="difficulty-filter">{['All','Easy','Medium','Hard'].map(item => <button key={item} className={difficulty === item ? 'selected' : ''} onClick={() => setDifficulty(item)}>{item}</button>)}</div></div>{filtered.length ? <div className="full-category-grid">{filtered.map((item, index) => { const Icon = iconMap[item.icon] || BrainCircuit; return <article className="full-category-card" key={item.id}><div className="card-top"><div className="category-icon"><Icon size={21} /></div><span className={`difficulty ${item.difficulty.toLowerCase()}`}>{item.difficulty}</span></div><span className="card-number">{String(index + 1).padStart(2, '0')}</span><h3>{item.name}</h3><p>{item.description}</p><div className="card-bottom"><span>{item.count}+ questions</span><button onClick={() => onStart(item.id)} aria-label={`Start ${item.name} practice`}>Start practice <ArrowRight size={15} /></button></div></article> })}</div> : <div className="empty-state"><Search size={28} /><h3>No subjects found</h3><p>Try another subject or difficulty.</p></div>}</section>
    </main><footer className="footer" id="about"><span>© 2026 BPSC TRE CS · Computer Science practice platform</span><div><a href="#categories">Subjects</a><a href="#mock">Full Mock</a><button className="footer-admin-btn" onClick={() => setAdminOpen(true)}>Admin</button></div></footer>
  </div>
}
